...
// TODO - CAUDAZ #7  Tune the following hyperparameters

#define INPUT_WIDTH   64        // 512
#define INPUT_HEIGHT  64        // 512
#define OPTIMIZER     "RMSprop" //"None"
#define LEARNING_RATE 0.15f     //0.0f  task#2 0.12f
#define REPLAY_MEMORY 10000     //10000
#define BATCH_SIZE    16        //8     task#2 10
#define USE_LSTM      true      //false
#define LSTM_SIZE     256       //32    task#2 256

// TODO - Define Reward Parameters

#define REWARD_WIN  +1.0f   //0.0f
#define REWARD_LOSS -1.0f   //-0.0f
...
bool ArmPlugin::createAgent()
{
	...
	agent = dqnAgent::Create(INPUT_WIDTH, INPUT_HEIGHT, INPUT_CHANNELS,
							 NUM_ACTIONS, OPTIMIZER, LEARNING_RATE, REPLAY_MEMORY, BATCH_SIZE, GAMMA,
...
void ArmPlugin::OnUpdate(const common::UpdateInfo& updateInfo)
{
	...
	if( hadNewState && !newReward )
	{
		...
		if(!checkGroundContact)
		{
			...
			const float distGoal = BoxDistance(gripper->GetBoundingBox(), prop->model->GetBoundingBox()); 
			...
			if( episodeFrames > 1 )
			{
				...
				avgGoalDelta  = avgGoalDelta * 0.2 + distDelta * (1.0 - 0.2);
